...
import classes from "./packageMapping.module.css";
...
const buildSvgSrc = (icon: string): string =>
  icon?.startsWith("http") ? icon : `/icons/${icon}`;

const filterMenuChildren = (children: MenuItem[]): MenuItem[] =>
  children.filter((child) => child.menuLevel === 3 && child.status !== 2);
  ...
  useEffect(() => {
    ...
    const fetchMenuDetails = async () => {
      try {
        ...
        console.log(res?.data?.children, "@Responsedata");
